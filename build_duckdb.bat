rem Usage build_duckdb SourceDir DestDir BuildDir Tool.
rem SourceDir (%1) Full path to the DuckDB repository.
rem DestDir (%2) Full path to the destination directory (Example: k:/duckdb/latest.
rem BuildDir (%3) Full path to a temporary directory where the build is done. Default %TEMP%/duckdb
...
if [%BUILD_DIR%] == [] set BUILD_DIR=%TEMP%\duckdb
...
cmake -G %GENERATOR% -S "%SOURCE_DIR%" -B "%BUILD_DIR%" -D CMAKE_INSTALL_PREFIX="%DEST_DIR%" ^
	  ...
	  -D BUILD_TESTING=OFF -D BUILD_UNITTESTS=OFF ^
      -D EXTENSION_STATIC_BUILD=ON


echo Build DuckDB library
cmake --build "%BUILD_DIR%"  --clean-first --parallel 24 --config %%C

echo Install DuckDB library
cmake  --install "%BUILD_DIR%" --config %%C --prefix "%DEST_DIR%"
)

echo Removing DuckDB build directory
rmdir "%BUILD_DIR%" /s /q