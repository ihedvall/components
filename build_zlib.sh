...
{
echo "Usage:"
echo "./build_zlib.sh SourceDir DestDir BuildDir"
echo "Where:"
...
echo "BuildDir:  [OPtional] Full path to a build root directory. Default is /tmp/zlib"
}
...
do
  ...
  cmake -G "Unix Makefiles" -S "$SOURCE_DIR" -B "$BUILD_DIR" -D CMAKE_INSTALL_PREFIX="$DEST_DIR" \
        -D CMAKE_BUILD_TYPE=$build_type -D CMAKE_DEBUG_POSTFIX=d
